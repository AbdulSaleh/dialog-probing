...
data_dir = Path(project_dir, 'data', 'probing', 'shuffle_across_dailydialog')
...
# Save files
question_path = data_dir.joinpath('shuffle_across_dailydialog.txt')
...
turns = []
for episode in data[int(len(data) / 2):]:
...
for i, episode in enumerate(data):
  ...
  for i in range(0, len(dialogue), 2):
    if not shuffle:
      ...
      utterance_1 = turns[turn_index]
      turn_index += 1
      if i == len(dialogue) - 1:
        utterance_2 = ''
      else:
        utterance_2 = turns[turn_index]
        turn_index += 1
